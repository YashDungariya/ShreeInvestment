...
const NomineeList = () => {
  ...
  const handleOpenNotesModal = (nominee) => {
    setNotesData({ id: nominee.id, nominee_notes: nominee.nominee_notes || "" });
    setOpenNotes(true);
  };
  ...
  const handleNotesSubmit = async () => {
    try {
      ...
      if (res.data.status === "success") {
        Swal.fire("Success!", "Nominee notes saved.", "success");
        setOpenNotes(false); fetchData();
      } else Swal.fire("Error", res.data.message || "Failed to save notes", "error");
    } catch (err) { Swal.fire("Error", "Server error while saving notes", "error"); }
  };
          ...
          {newNominees.map((nom, index) => (
            <Box key={index} sx={{ mb: 3, p: 3, border: "1px solid #e2e8f0", borderRadius: 2, position: "relative" }}>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}><TextField label="Bank Details" fullWidth size="small" value={nom.bankDetails} onChange={(e) => handleNewNomineeChange(index, "bankDetails", e.target.value)} /></Grid>
                <Grid item xs={12}><TextField label="Notes" fullWidth size="small" multiline minRows={2} value={nom.nomineeNotes} onChange={(e) => handleNewNomineeChange(index, "nomineeNotes", e.target.value)} /></Grid>
              </Grid>
            </Box>
          ))}
